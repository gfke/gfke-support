...
class BaseUrmFacade {
    ...
    emit(event) {
        ...
        _logger2.default.debug('Emit WebSocket event', event, data);

        _socket2.default.emit(event, this.appendTokenTo(data));

        return new _bluebird2.default((resolve, reject) => {
            //TODO: Check if this might lead to memory leak as a new handler is registered each time this is called
            _socket2.default.on(event, data => {
                try {
                    if (data.status) {
                        resolve(transformFn.call(null, data));
                    } else {
                        reject(new _socket.SocketError(data.message, data.statusCode, stack));
                    }
                } catch (e) {
                    reject(e);
                }
            ...
            throw new _socket.SocketError('Socket event timed out - assuming wrong event', 500, stack);
        }).catch(error => {
            ...
            throw new _socket.SocketError(error, 500, stack);
        });
    }
}
