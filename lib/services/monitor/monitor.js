...
const debug = require('../../utils/debug')('gfke-support:monitor:monitor');
...
module.exports = class Monitor {
    ...
    collectSystemMetrics() {
        return co(function*() {
            ...
            if (_.has(this.strategy.opts, 'systemMetrics')) {
                metrics = _.pick(metrics, this.strategy.opts.systemMetrics);
            }

            this.log.debug('systemMetrics: %j', metrics);
            if (_.has(this.strategy.opts, 'log')) {
                this.strategy.opts.log.info('systemMetrics', metrics);
            }
    ...
    collectProcessMetrics() {
        ...
        if (_.has(this.strategy.opts, 'processMetrics')) {
            metrics = _.pick(metrics, this.strategy.opts.processMetrics);
        }

        this.log.debug('processMetrics: %j', metrics);
        if (_.has(this.strategy.opts, 'log')) {
            this.strategy.opts.log.info('processMetrics', metrics);
        }
