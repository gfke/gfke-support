...
module.exports = class DwhApiService {
    ...
    setupDwhApiMethods () {
        _.forEach(this._options.paths, (methods, action) => {
            ...
            methods.forEach(m => {
                ...
                this[queryRMethod] = (suffix, options, callback) => {
                    return this.queryR(action, m, suffix, options, callback);
                };
    ...
    query (token, model, dataPath, dataSource, suffix, transformFn, transformErr) {
        var context;
        if (!_.isUndefined(token.context)) {
        ...
        var domainWithContext;
        if (context !== null) {
            domainWithContext = this.domain + '.' + context;
        } else {
            domainWithContext = this.domain;
        }
    ...
    queryR(dataPath, dataSource, suffix, options, callback) {
        if (dataPath == null) {
            dataPath = '';
        }
        ...
        this.log.info("Run Dwh Api Query for uri: %s", uri);
        this.log.info("Options: %j", options);
