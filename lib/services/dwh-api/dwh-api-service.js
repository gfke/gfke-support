...
module.exports = class DwhApiService {
    ...
    setupDwhApiMethods () {
        _.forEach(this._options.paths, (methods, action) => {
            ...
            methods.forEach(m => {
                ...
                this[queryRMethod] = (token, suffix, options, callback) => {
                    return this.queryR(token, action, m, suffix, options, callback);
                };
    ...
    queryR(token, dataPath, dataSource, suffix, options, callback) {
        if (dataPath == null) {
            dataPath = '';
        }
