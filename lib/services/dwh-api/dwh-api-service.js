...
module.exports = class DwhApiService {
    ...
    setupDwhApiMethods () {
        _.forEach(this._options.paths, (methods, action) => {
            ...
            methods.forEach(m => {
                ...
                this.debug('setup %s for %s', queryRMethod, m);
                this[queryRMethod] = (token, data, suffix, options, callback) => {
                    return this.queryR(token, data, action, m, suffix, options, callback);
                };
                this._methods[m] = this[queryMethod];
                this._rMethods[m] = this[queryRMethod];
            });
        });
    }
    ...
    queryR(token, model, dataPath, dataSource, suffix, options, callback) {
        if (dataPath == null) {
            dataPath = '';
        }
