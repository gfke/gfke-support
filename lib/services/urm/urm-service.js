    ...
    socket: {
        ...
        events: {
            ...
            connect_error: function(error) {
                this.log.debug('socket - connection error');
                // debug('url was %s', socketOptions.url);
                this.log.debug(error);
            },
            error:function(error) {
                this.log.debug('socket - error %s', error);
            }
            // connect_timeout:
            // reconnect:
            // reconnect_attempt:
...
exports = module.exports = class URMService {
    ...
    initSocket() {
        this.log.debug('setup urm socket');
        ...
        const usedSocketClient = this._options.socket.proxy !== false
            ? socketProxy
            : socketClient;

        if (_.isString(this._options.socket.proxy)) {
            socketProxy.init(this._options.socket.proxy)
        }
        ...
        _.forEach(this._options.socket.events, (v, k) => {
            this.log.silly('setup urm socket event %s', k);
