...
exports = module.exports = class URMService {
    ...
    constructor(options) {
        ...
        debug('setup urm');
        debug('setup urm url: %s', this._options.url);
        this.initSocket();
        this.setupUrmMethods();
    }

    initSocket() {
        if (!this._options.socket.url) {
            debug('No urm socket url given. Skipping urm socket setup');
            return;
        }
        debug('setup urm socket url: %s', this._options.socket.url);
        debug('setup urm socket baseEvent: %s', this._options.socket.baseEvent);
        debug('setup urm socket options: %j', this._options.socket.options);

        this._socket = this._options.socket.socketClient(
    ...
    query(action, route, data, transformFn, transformErr) {
        ...
        debug("Run URM Query for uri: %s", uri);
    ...
    emit(action, event, data, transformFn, transformErr) {
        ...
        debug('Emit URM Socket event %s', event);
