...
module.exports = function* longPoll(next) {
    ...
    try {
        yield next;
        this.chunkedResponse.push(JSON.stringify(this.state.longPollBody || ' '));
    } catch (e) {
        ...
        const resp = {
            message: e.message,
            error: e,
            stack: this.app.env === 'development'? e.stack.split("\n") : []
        }; 
        this.chunkedResponse.push(JSON.stringify(resp));
    } finally {
